$(function() {
	...
	var surl = '/warehouse/findCdList.htm';
	...
	var columndefs = [                             
	  	            {"render": function ( data, type, row,index ) {
		                ...
		                var html = '';
		                if(enablePay == 1){
		                	html =  '<input type="button" value="仓单转出" class="J_Show cbtn" data-key="'+index.row+ '" />';
		                }
...
//    });
	...
	$('#confirmbtn1').on('click', function(event){
		...
		$.ajax({
			type : 'POST',
			url : '/warehouse/rollout.htm',
			data:"registno="+$("#registnotmp").html(),
			cache : false,
			dataType : 'json',
			success : function(res) {
				if(res.succflag == '0'){
					UI.Dialog({type : 'tips',width : 320, title : '撤销提示',content : "注册仓单转出成功！"}).show();
					UP.Dialog('J_cdList').close();
					$('.mask').hide();
					$('.mark').remove();
					dataList.ajax.reload();
