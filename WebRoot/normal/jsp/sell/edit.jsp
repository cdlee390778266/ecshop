...
<link type="text/css" rel="stylesheet"
	href="/normal/widget/css/ui.datepicker.css" />
...
<link type="text/css" rel="stylesheet"
	href="/normal/css/combo.select.css">
...
<script type="text/javascript"
	src="/normal/js/handle.sell.edit.js?v=${sessionScope.buildno}"></script>
<title>卖方挂牌</title>
</head>
<body>
		...
		<div class="grid-16-16">
			<div class="crumb-nav">
				<div class="crumb">
					<a href="/home.htm">交易大厅</a><span class="fa  fa-angle-right"></span><a
						href="/sell/apply.htm?active=enter">卖方挂牌</a>
				</div>
			</div>
			...
			<div class="page">
				...
				<div class="main-content">
					<div class="bd">
						<form action="/sell/edithandle.htm" method="post"
							enctype="multipart/form-data" id="sellEdit">

							<input type="hidden" name="busDate" id="busDate"
								value="${busDate}" />
							...
							<div class="page-module warehouse">
								<div class="row">
									...
									<div class="bd">
										<table class="ui-table marl20 marr20 mart20">
											<tbody>
												<tr>

													<td class="ctr" width="125px">挂牌单号：</td>
													<td colspan="3">${rspBody.listedNo}<input
														type="hidden" name="listedNo" value="${rspBody.listedNo}" />
												...
												<tr>
													...
													<td colspan="3"><c:if
															test="${rspBody.listedType=='M'}">
															<span class="ml20"><input type="radio"
																name="listedType" value="M" checked />保证金</span>
														</c:if> <c:if test="${rspBody.listedType=='W'}">
															<span class="ml20"><input type="radio"
																name="listedType" value="W" checked />注册仓单</span>
														</c:if></td>
												</tr>
												...
												<tr>
													...
													<td colspan="3"><input type="hidden" name="commCode"
														id="commCode" value="${rspBody.commCode}" />
														...
														<c:forEach items="${rspBody.props}" var="prop"
															...
															<c:choose>
																<c:when test="${prop.consType != '81'}">
																	...
																	<td class="ctr" width="125px"><c:if
																			test="${prop.forceInput =='1'}">
																${prop.propName}<span class="forceinput">(必填)</span>：
															</c:if> <c:if test="${prop.forceInput !='1'}">
																${prop.propName}：
															</c:if></td>
																	<td><c:if test="${prop.consType == '00'}">
																			...
																			<c:if test="${!empty fmt && fn:length(fmt) > 0}">
																				<c:if test="${fn:indexOf(fmt, '||') >= 0}">
																					<c:set var="vunit"
																						value="${fn:substringAfter(fmt, '||')}" />
																					<c:set var="fmt"
																						value="${fn:substringBefore(fmt, '||')}" />
																				</c:if>
																				<c:set var="vfmt"
																					value="${fn:substringAfter(fmt, 'fm')}" />
																				<c:set var="pointidx"
																					value="${fn:indexOf(vfmt, '.')}" />
																			...
																			<c:if test="${!empty vunit && fn:length(vunit)>0}">
																				<c:set var="pval"
																					value="${fn:substringBefore(prop.propVal, vunit)}" />
																			</c:if>

																			<c:if test="${prop.valType =='CUR'}">
																				<c:set var="valid" value="${valid}${'isMoney||'}" />
																				<c:set var="validmsg"
																					value="${validmsg}${'输入格式货币类型||'}" />
																			</c:if>

																			<c:if test="${prop.valType =='DAT'}">
																				<c:set var="valid" value="${valid}${'isDate||'}" />
																				<c:set var="validmsg"
																					value="${validmsg}${'输入格式日期类型||'}" />
																				<c:set var="extmsg"
																					value="${'datepicker data-date-format=yyyy-mm-dd data-auto-close=true'}" />
																			</c:if>
																			<c:if test="${prop.valType =='TIM'}">
																				<c:set var="valid" value="${valid}${'isTime||'}" />
																				<c:set var="validmsg"
																					value="${validmsg}${'输入格式时间类型||'}" />
																			</c:if>
																			<c:if test="${prop.valType =='NUM'}">
																				<c:if test="${fmtlength > 0}">
																					<c:if test="${pointidx < 0}">
																						<c:set var="valid"
																							value="${valid}${'isRegexpInt:'}${fmtlength}${'||'}" />
																						<c:set var="validmsg"
																							value="${validmsg}${'输入格式不正确||'}" />
																					</c:if>
																					<c:if test="${pointidx >= 0}">
																						<c:set var="valid"
																							value="${valid}${'isRegexpNum:'}${fmtlength}${'-'}${fmtlength-pointidx-1}${'||'}" />
																						<c:set var="validmsg"
																							value="${validmsg}${'输入格式不正确||'}" />
																					</c:if>
																					<c:set var="valid" value="${valid}${'onlyNum||'}" />
																					<c:set var="validmsg"
																						value="${validmsg}${'输入格式只能为数字||'}" />
																				</c:if>
																				<c:if test="${fmtlength > 0}">
																					<c:set var="valid"
																						value="${valid}${'betweenval:0-'}${vfmt}${'||'}" />
																					<c:set var="validmsg"
																						value="${validmsg}${'输入超过限值||'}" />
																				</c:if>
																			</c:if>
																			<c:if test="${prop.valType =='PER'}">
																				...
																				<c:if test="${fmtlength > 0}">
																					<c:set var="valid"
																						value="${valid}${'betweenval:0-100||'}" />
																					<c:set var="validmsg"
																						value="${validmsg}${'输入超过限值||'}" />
																				</c:if>
																			</c:if>
																			<c:set var="valid"
																				value="${valid}${'maxGBLength:128'}" />
																			<c:set var="validmsg"
																				value="${validmsg}${'长度不能超过128'}" />

																			<div class="form_control">
																				<input type="text" name="propdata" id="propdata"
																					class="required" ${extmsg} value="${pval}"
																					data-empty="${prop.forceInput}" id="propdata"
																					data-unit="${vunit}" data-key="${prop.propIdx}"
																					data-tip="${tipsmsg}" autocomplete="off"
																					data-valid="${valid}" data-error="${validmsg}" />
																				<c:if test="${!empty vunit && fn:length(vunit)>0}">
																					<span class="unit_message">${vunit}</span>
																				</c:if>
																			</div>
																		</c:if> <c:if test="${prop.consType == '01'}">
																			<input type="hidden" name="propdata" value=""
																				data-key="${prop.propIdx}" data-unit="" />
																			<c:set var="enumvals"
																				value="${fn:split(prop.consVal, ';')}" />
																			<select name="propsel" id="propsel" class="csel"
																				data-key="${prop.propIdx}"
																				data-empty="${prop.forceInput}"
																		...
																		</c:if> <c:if test="${prop.consType == '02'}">
																			<input type="hidden" name="propdata" value=""
																				data-key="${prop.propIdx}" data-unit="" />
																			<select name="propsel" id="propsel" class="csel"
																				data-key="${prop.propIdx}"
																			...
																			<span class="valid_message"></span>
																		</c:if> <c:if test="${prop.consType == '03'}">

																			<c:set var="keyoffset"
																				value="${fn:indexOf(prop.consVal, ')')}" />
																			<c:set var="relakey"
																				value="${fn:substring(prop.consVal, 1, keyoffset)}" />
																			<c:set var="items"
																				value="${fn:substringAfter(prop.consVal, ')')}" />
																			...
																			<c:set var="nprefix"
																				value="${fn:substringAfter(items, prefix)}" />
																			<c:set var="seles"
																				value="${fn:substringBefore(nprefix, '}')}" />
																			<c:set var="enumvals" value="${fn:split(seles, ';')}" />

																			<input type="hidden" name="propdata" value=""
																				data-key="${prop.propIdx}" data-unit="" />
																			<select name="propsel" id="propsel" class="csel"
																				data-type="${prop.consType}"
																				data-key="${prop.propIdx}"
																				data-relaname="${keyname}" data-relakey="${relakey}"
																				data-items="${items}"
																				data-empty="${prop.forceInput}"
																				data-propname="${prop.propName}">
																				...
																				<c:choose>
																					<c:when
																						test="${empty seles || fn:length(seles) == 0}">
												...
												<tr>
													...
													<td width="285px">
														<div class="form_control">
															<input type="text" name="unitPrice" class="required"
																id="unitPrice" value="${rspBody.up}"
																data-limit="${pricelimit}" data-tip="请输入商品单价"
																data-valid="isNonEmpty||isNoNZeroMoney"
																data-error="单价必填||金额格式:1.00" /> <span
																class="priceunit_message">元/${rspBody.uom}</span>
														</div>
													</td>
													...
													<td><span class="ml20 mr20"><input type="radio"
														...
														<input type="hidden" name="fpFlag" value="F" /></td>
												</tr>
															...
															<c:choose>
																<c:when test="${rspBody.wholeFlg == 'W'}">
																	<span class="ml20 mr20"><input type="radio"
																		name="wholeFlag" id="w_flag" value="W" checked />是</span>
																	<span><input type="radio" name="wholeFlag"
																		id="s_flag" value="S" />否</span>
																</c:when>
																<c:otherwise>
																	<span class="ml20 mr20"><input type="radio"
																		name="wholeFlag" id="w_flag" value="W" />是</span>
																	<span><input type="radio" name="wholeFlag"
																		id="s_flag" value="S" checked />否</span>
																</c:otherwise>
															</c:choose>
														</div>
												...
												<c:if test="${rspBody.listedType=='M'}">
													<tr>
														...
														<td class="ctr">起订数量<span class="forceinput">(必填)</span>：
														</td>
														<td width="285px">
															<div class="form_control">
																<input type="text" name="moq" id="moq" class="required"
																	value="${rspBody.moq}"
																	<c:if test="${rspBody.wholeFlg == 'W'}"> readonly="true" </c:if>
																	data-tip="请输入商品起订数量" data-valid="isNonEmpty||plusInt"
																	data-error="起订量必填||起订量必须为整数" /> <span
																	class="unit_message">${rspBody.uom}</span>
															</div>
														</td>
														<td class="ctr">递增数量<span class="forceinput">(必填)</span>：
														</td>
														<td>
															<div class="form_control">
																<input type="text" name="incrNum" id="incrNum"
																	class="required" value="${rspBody.ic}"
																	<c:if test="${rspBody.wholeFlg == 'W'}"> readonly="true" </c:if>
																	data-tip="请输入商品递增量" data-valid="isNonEmpty||plusInt"
																	data-error="递增量必填||递增量必须为整数" /> <span
																	class="unit_message">${rspBody.uom}</span>
															</div>
														</td>
													...
													<input type="text" name="doe" id="doe" value="${rspBody.doe}"  datepicker data-date-format="yyyy-mm-dd" data-auto-close="true" class="required"  data-tip="请选择挂牌有效期" data-valid="isNonEmpty||isDate||after:${busDate}" data-error="挂牌有效期必填||有效期格式不正确||有效期小于业务日期${busDate}"  />
													 -->
															<input type="text" name="doe" id="doe"
																value="${rspBody.doe}" data-min="${busDate}"
																class="required" data-tip="请选择挂牌有效期"
											...
											<c:if test="${rspBody.listedType=='M'}">
												<tr>
													...
													<td>合同签定后 <input type="text" name="lastPD" id="lastPD"
														maxlength="4"
														value="${fn:substringAfter(rspBody.lastPD,'cycle:')}"
													...
													<td>全款支付后 <input type="text" name="deliDate"
														...
														value="${fn:substringAfter(rspBody.deliDate,'cycle:')}"
														style="width: 40px; height: 24px; padding: 5px 5px; border: 1px solid #ECECEC;"
														autocomplete="off" />天 <span class="valid_message"></span>
													</td>
												</tr>
												...
												<tr>
													<td class="ctr">交收仓库<span class="forceinput">(必填)</span>：
													</td>
													...
													</select> <span class="valid_message"></span></td>
													<td class="ctr">平台监管发票：</td>
													<td><c:choose>
															<c:when test="${rspBody.invoice == 'Y'}">
																<span class="ml20 mr20"><input type="radio"
																	name="invoice" value="Y" checked />需要</span>
																<span><input type="radio" name="invoice"
																	value="N" />不需要</span>
															</c:when>
															<c:otherwise>
																<span class="ml20 mr20"><input type="radio"
																	name="invoice" value="Y" />需要</span>
																<span><input type="radio" name="invoice"
																	value="N" checked />不需要</span>
															</c:otherwise>
														</c:choose></td>
												</tr>
												...
												<tr>
													...
													<td colspan="3"><c:set var="delistIDs" value="${''}" />
														<c:set var="delistNames" value="${''}" /> <c:choose>
															<c:when test="${rspBody.delist =='O'}">
																<span class="ml20 mr20"><input type="radio"
																	name="delist" value="O" checked />不指定</span>
															...
															<c:otherwise>
																<span class="ml20 mr20"><input type="radio"
																	name="delist" value="O" />不指定</span>
																<span><input type="radio" name="delist" value="A"
																	checked />指定</span>
																...
																<c:forEach items="${rspBody.delistMems}" var="dm">
																	<c:set var="delistIDs"
																		value="${delistIDs}${dm.delistMID}${';'}" />
																	<c:set var="delistNames"
																		value="${delistNames}${dm.delistMemName}${';'}" />
																</c:forEach>


															</c:otherwise>
														</c:choose> <input type="hidden" name="markcode" id="markcode"
														value="${rspBody.markCode}" /> <input type="hidden"
														name="memdelists" id="memdelists" value="${delistIDs}"
														data-key="${delistNames}" /></td>
												</tr>
													...
													<td>
														<div class="form_control">
															<input type="text" name="title" class="required"
																value="${rspBody.title}" data-tip="请输入描述标题"
																data-valid="maxGBLength:128" data-error="长度不超过128" />
														</div>
													</td>
																	...
																	<div class="clearfix">
																		<c:if test="${fn:length(rspBody.titlePic)> 0 }">
																			<img id="titfilePre" width="120px" height="120px"
																				src="${rspBody.titlePic}"
																				style="display:; float: left" />
																			<span class="btn-upload_unselected fileinput ml10">
																				<span>替换标题图片</span> <input type="file"
																				name="titfile" id="titfile"
																		...
																		<c:if test="${fn:length(rspBody.titlePic)== 0 }">
																			...
																			<span class="btn-upload_unselected fileinput ml10">
																				<span>选择标题图片</span> <input type="file"
																				name="titfile" id="titfile"
																				accept="image/jpg,image/jpeg,image/png,image/bmp,image/gif" />
																			</span>
																		</c:if>
																	...
																	<div class="clearfix">
																		...
																		<c:if test="${fn:length(rspBody.ctxPic1)> 0 }">
																			<img id="ctxPic0Pre" width="120px" height="120px"
																				...
																				style="display:; float: left" />
																			<span class="btn-upload_unselected fileinput ml10">
																				<span>替换商品描述首图</span> <input type="file"
																	...
																	<div class="clearfix">
																		...
																		<c:if test="${fn:length(rspBody.ctxPic2)> 0 }">
																			<img id="ctxPic1Pre" width="120px" height="120px"
																				...
																				style="display:; float: left" />
																			<span class="btn-upload_unselected fileinput ml10">
																				<span>替换商品描述次图</span> <input type="file"
																	...
																	<div class="clearfix">
																		...
																		<c:if test="${fn:length(rspBody.ctxPic3)> 0 }">
																			<img id="ctxPic2Pre" width="120px" height="120px"
																				...
																				style="display:; float: left" />
																			<span class="btn-upload_unselected fileinput ml10">
																				<span>替换商品描述尾图</span> <input type="file"
										...
										<div class="pa-action clearfix mt10">
											<div class="pa-btn-sell fn-fl">
												<button class="btn-normal btn-sell"
													style="margin-left: 200px;">修改挂牌</button>
											</div>
											<div></div>
										</div>
									</div>
