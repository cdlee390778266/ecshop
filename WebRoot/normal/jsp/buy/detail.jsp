...
<link type="text/css" href="/normal/css/square/green.css"
	rel="stylesheet">
	...
	<div class="wrapper">
			...
			<div class="crumb-nav">
				<div class="crumb">
					<a href="/home.htm">交易大厅</a><span class="fa  fa-angle-right"></span><a
						href="/buy/list.htm">会员中心</a><span class="fa  fa-angle-right"></span><a
						href="/buy/list.htm">购买订单</a>
				</div>
			</div>
			<!-- main -->
						...
						<div class="page-module bsmenus">
									...
									<table class="ui-table bstable">
										<tbody>
											...
											<tr>
												...
												<td>${rspBody.commName}<span style="color: #F40">${rspBody.title}</span>
												</td>
												<c:forEach items="${rspBody.props}" var="prop">
													<c:if
														test="${prop.propName =='品牌' || prop.propName =='产地' }">
											...
											<tr>
												...
												<td><fmt:formatNumber value="${rspBody.up}"
														type="currency" pattern="￥0.00元" />/${rspBody.uom}</td>
							...
							${rspBody.lastPD}
											...
											<tr>
												...
												<td><c:if test="${rspBody.top =='S'}">
					仅付定金

				</c:if> <c:if test="${rspBody.top =='F'}">
				全款支付
		...
		交易平台不负责监管发票
							...
							<div class="row bsrow">
								...
								<div class="box">
									<table class="ui-table table-primary" style="margin-top:0px;">
										<thead>
											<tr>
												<td>费用名称</td>
												<td>交易类型</td>
										...
										<tbody>
											<c:forEach items="${rspBody.costPays}" var="cost">
												<tr>
													...
													<td class="fcyellow"><fmt:formatNumber
															value="${cost.costAmt}" type="currency" pattern="￥0.00元" /></td>
													<td>${cost.flagDesc}</td>
												</tr>
											</c:forEach>
										</tbody>
							...
							<c:if test="${operType=='A'||operType=='P'}">
								...
								<div class="row bsrow">
									<form name="buyform" id="buyform" method="post"
										action="/buy/${link}.htm">
										<input type="hidden" name="delistNo"
											value="${rspBody.deListNo}" />
										<div class="bd">
											<table class="ui-table bstable new-table">
												<c:if test="${operType=='A'}">
													...
													<tr>
														<td colspan="2"><label for="">审核意见：</label>
															<div class="form_control new-control">
																<textarea name="comment" id="comment" type="text"
																	class="required" data-tip="请输入审核意见"
																	data-valid="maxGBLength:512" data-error="审核意见长度错误" /></textarea>
															</div></td>
													</tr>

													<tr>
														<td colspan="2">
															<div class="pa-btn-buy fr marr100">
											...
											<c:if test="${operType=='P'}">
												<table class="ui-table bstable pay-table mart20">
													<tr>
														...
														<td width="240px">
															<div class="form_control new-pay" style="top: -4px;">
